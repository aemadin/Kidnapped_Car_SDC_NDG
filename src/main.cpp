        ...
        if (event == "telemetry") {
          ...
          if (!pf.initialized()) {
            ...
            double sense_theta = std::stod(j[1]["sense_theta"].get<string>());
              //added for debugginh/understanding 
            std::cout <<"sense_x "<< sense_x<< " "<<"sense_y "<< sense_y<< " "<<"sense_theta "<< sense_theta<<  std::endl;
          ...
          for (int i = 0; i < x_sense.size(); ++i) {
            ...
            obs.y = y_sense[i];
            //added for debugginh/understanding 
            //std::cout <<i<<"th obser "<< obs.x<< " "<<obs.y << std::endl;
            noisy_observations.push_back(obs);
          }
			
          // Update the weights and resample
          pf.updateWeights(sensor_range, sigma_landmark, noisy_observations, map);
          // for debug std::cout <<" passed Updating weights"<< std::endl;
          pf.resample();
