...
//added by me
inline LandmarkObs TransformObs(Particle particle, LandmarkObs obs) {
	LandmarkObs transformed;
	transformed.x = particle.x + (cos(particle.theta) * obs.x) - (sin(particle.theta) * obs.y);
	transformed.y = particle.y + (sin(particle.theta) * obs.x) + (cos(particle.theta) * obs.y);
   
	return transformed;}

inline double MultivProb(double sig_x, double sig_y, double x_obs, double y_obs,
